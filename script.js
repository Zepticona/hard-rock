...
document.getElementById('search-button').addEventListener('click', () => {
    ...
    .then(returnedData => {
        ...
        resultArray.forEach((el, i) => {
            let title, artist, album, searchResultHtml;
            
            title = returnedData.data[i].title;
            artist = returnedData.data[i].artist.name;
            album = returnedData.data[i].album.title;
            ...
            searchResultList[i].addEventListener('click', () => {
                ...
                .then(response => {
                    ...
                    if(response.ok === false) {
                        document.getElementById(`whole-lyrics-${i}`).innerHTML = "Sorry, no lyrics found on the database";
                        return;
                    }
